...
\author{Lee De Zhang}
\title{Coupling and Poisson Approximation}
...
\begin{itemize}
	\item Given an irreducible, aperiodic Markov chain $X$,
	 ...
	 \begin{itemize}
	 	...
	 	\]
	 	for any $x \in S$ 
	 	\item Equivalently, $\langle \mu, \Pi f \rangle = \langle \mu, f \rangle$ for any bounded $f$
	 \end{itemize}
...
\begin{frame}
	\begin{itemize}
		\item Let $X^1$ be an independent copy of $X$ with initial distribution $\mu$
		\begin{itemize}
			\item That is, we pick the starting point of $X^1$ from $S$ using $\mu$
		\end{itemize}
		\item Let $X^2$ be an independent copy of $X$ with initial distribution $\delta_x$ for some $x \in S$
		\begin{itemize}
			\item $\delta_x$ is the Dirac measure
			\item Equivalently, this means we start $X^2$ at $x$ with probability 1
		\end{itemize}
		\item $(X^1, X^2)$ form a coupling on state space $(S \times S)$
...
\begin{frame} {$X^1$ and $X^2$ meet at $\tau < \infty$ a.s.}
	\begin{itemize}
		...
		\begin{itemize}
			\item Irreducibility means, for any $w, x, y, z \in S$,
			\[
...
\begin{itemize}
	\item Define two new Markov chains $\tilde{X}^1, \tilde{X}^2$
		\begin{itemize}
			...
			\begin{itemize}
				\item i.e. when both $X^1$ and $X^2$ meet, the second MC follows the first MC
			\end{itemize}
		\end{itemize}
...
\begin{frame}{Stein's Method}
	\begin{itemize}
		...
		\item Appeal is in its abstractness. Can generalize it to other distributions
		\begin{itemize}
			\item Stein-Chen method generalizes it to Poisson approximation
...
\begin{frame}{General Framework of Stein's Method}
		...
		\begin{definition}[\emph{Total Variation Distance}]
			Let  $P$ and $Q$ be probability measures on the same probability space $(\Omega, \mathcal{F})$.
...
\section{Stein-Chen Method}

\begin{frame}{Stein-Chen Method}
\begin{itemize}
	\item An extension of Stein's method by his student, Louis Chen in his paper \cite{chen1975poisson}
	\item Using the same techniques as the Stein method, but using Poisson distribution instead of Normal
...
\begin{itemize}
	\item $r$ balls are thrown independently and randomly at $n$ boxes with equal probability
	\begin{itemize}
	...
	\end{itemize}
	\item Let $p_\alpha$ be the probability that box $\alpha$ is still empty
	\begin{itemize}
		\item Equivalently, $p_\alpha := \mathbb{P}(I_\alpha = 1) = \left(1 - \frac{1}{n}\right)^r$
	\end{itemize}
...
\begin{itemize}
	...
	\[
	\begin{split}
	d_{TV}(\mathcal{L}(W), Po(\lambda)) & \leq \lambda  - Var(W)  \\
	& =  \sum_\alpha p_\alpha [1-(1-p_\alpha)] \\
	& = \sum_\alpha p_\alpha^2 \\
	&= n \left(1 - \frac{1}{n}\right)^r \\
	& \leq n e^{-r/n},
	\end{split}
	\]
	where we used the approximation $\left(1 - \frac{1}{n}\right)^n \leq e^{-n}$ for sufficiently large $n$.
\end{itemize}
\end{frame}

\begin{frame}
	\textbf{Question: Is Poisson approximation justified here?} 
	\begin{itemize}
		\item Since $d_{TV}(\mathcal{L}(W), Po(\lambda)) \leq n e^{-r/n}$, 
		\begin{itemize}
			\item A sufficient condition for $d_{TV} \rightarrow 0$ is $r >> n$
		\end{itemize}
		\item Consistent with what we know about the law of small numbers
		\begin{itemize}
			\item $p_\alpha = \left(1 - \frac{1}{n}\right)^r \leq e^{-r/n} \rightarrow 0$ for $r>>n$
			\item Poisson approximation works well for `rare' events (i.e. small $p_\alpha$)
		\end{itemize}
		\item Poisson approximation justified using Stein-Chen method! 
	\end{itemize}
\end{frame}
...
\begin{itemize}
	\item Probabilistic coupling is a powerful tool in probability theory
	\begin{itemize}
		\item Enables sleek and elegant proofs!
	\end{itemize}
	\item Coupling is an indispensable tool in the Stein-Chen method
	\begin{itemize}
		\item A good coupling choice can be used to justify using Poisson approximations
	\end{itemize}
\end{itemize}
\end{frame}
